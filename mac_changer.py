...
def menu():
    ...
    while True:
        ...
        try:
            menu = int(input("choose network interface:\t")) #menu to choose listed available nics
            if menu<0 or menu>len(interfaces)-1:
                ...
                if change_mac_address == "Y":
                    ...
                    nic = getnic.ipaddr(interfaces)  # generates dictionary with nics info
                    nic = nic.get(interfaces[menu])  # exporting user choosen nic info into a dictionary
                    ...
                    if exit=="Y": #exit loop or continue
                        break
                    if exit!="N":
                        print("wrong option")
        except ValueError:
